...
@router.post("/movies", status_code=status.HTTP_201_CREATED, response_model=MoviesSchema)
@inject
...
@router.get("/movies/{movie_id}", status_code=status.HTTP_200_OK, response_model=MoviesSchema, responses={
...
@router.get("/movies", status_code=status.HTTP_200_OK, response_model=List[MoviesSchema])
@inject
...
@router.put("/movies/{movie_id}", status_code=status.HTTP_204_NO_CONTENT, responses={
    404: {"description": "Movies not found"}
    })
@inject
...
@router.delete("/movies/{movie_id}", status_code=status.HTTP_204_NO_CONTENT, responses={
    404: {"description": "Movies not found"}
    })
@inject
